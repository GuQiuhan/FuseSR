...
class checkpoint():
    ...
    def save(self, trainer, epoch, is_best=False):
        trainer.model.save(self.dir, epoch, is_best=is_best) # 保存model
        trainer.loss.save(self.dir) # 保存loss
        trainer.loss.plot_loss(self.dir, epoch) # 保存可视化图

        self.plot_psnr(epoch)
        torch.save(self.log, os.path.join(self.dir, 'psnr_log.pt')) #保存psnr
        torch.save(
    ...
    def save_results(self, filename, save_list, scale): #归一化，转为numpy数组，存储为图像
...
def quantize(img, rgb_range): # 量化为整数
